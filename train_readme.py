...
def preprocess(text, stopwords, tokenized=True, lowercased=True):
...
def pre_process_features(X, y, stopwords, tokenized=True, lowercased=True):
    X = [preprocess(str(p), stopwords, tokenized=tokenized, lowercased=lowercased) for p in list(X)]  # Tiền xử lý dữ liệu
...
def load_data(data_path, dev_data_path, test_data_path):
    train_data = pd.read_csv(data_path)  # Đọc dữ liệu huấn luyện
    dev_data = pd.read_csv(dev_data_path)  # Đọc dữ liệu phát triển
    test_data = pd.read_csv(test_data_path)  # Đọc dữ liệu kiểm tra
...
def create_tokenizer(train_X):
    tokenizer = text.Tokenizer(lower=False, filters='!"#$%&()*+,-./:;<=>?@[\\]^`{|}~\t\n')  # Khởi tạo tokenizer
    tokenizer.fit_on_texts(train_X)  # Huấn luyện tokenizer trên dữ liệu huấn luyện
    return tokenizer

def make_features(X, y, tokenizer, sequence_length, is_one_hot_label=True):
...
def build_model(sequence_length, num_words, embedding_dim, embedding_matrix, num_filters, filter_sizes, drop):
...
if __name__ == "__main__":
    # --- 1. Định nghĩa các thông số và đường dẫn ---
    ...
    # --- 3. Tải dữ liệu ---
    X_train, y_train, X_dev, y_dev, X_test, y_test = load_data(DATA, DEV_DATA, TEST_DATA)

    # --- 4. Tiền xử lý dữ liệu ---
    train_X, train_y = pre_process_features(X_train, y_train, stopwords, tokenized=True, lowercased=True)  # Tiền xử lý dữ liệu huấn luyện
    dev_X, dev_y = pre_process_features(X_dev, y_dev, stopwords, tokenized=True, lowercased=True)  # Tiền xử lý dữ liệu phát triển
    test_X, test_y = pre_process_features(X_test, y_test, stopwords, tokenized=True, lowercased=True)  # Tiền xử lý dữ liệu kiểm tra
    ...
    embedding_matrix = np.zeros((num_words, 300))  # Khởi tạo ma trận nhúng

    for word, i in word_index.items():
        if i >= 20000:  # Kiểm tra nếu chỉ số từ lớn hơn kích thước từ vựng
            continue
    ...
    # --- 8. Tạo các đặc trưng cho mô hình ---
    train_X, train_y = make_features(train_X, train_y, tokenizer, sequence_length=150)  # Chuẩn bị dữ liệu huấn luyện
    dev_X, dev_y = make_features(dev_X, dev_y, tokenizer, sequence_length=150)  # Chuẩn bị dữ liệu phát triển
    test_X, test_y = make_features(test_X, test_y, tokenizer, sequence_length=150, is_one_hot_label=False)  # Chuẩn bị dữ liệu kiểm tra

    # --- 9. Xây dựng mô hình ---
    model = build_model(sequence_length=150, num_words=num_words, embedding_dim=300, embedding_matrix=embedding_matrix, num_filters=64, filter_sizes=[3, 4, 5], drop=0.4)

    # --- 10. Biên dịch mô hình ---
    ...
    model.fit(train_X, train_y, batch_size=512, epochs=60, verbose=1, validation_data=(dev_X, dev_y))  # Huấn luyện mô hình
    model.save(MODEL_FILE)  # Lưu mô hình

    # --- 12. Đánh giá mô hình ---
    cf1 = evaluate_model(model, test_X, test_y, batch_size=512)  # Đánh giá mô hình

    # --- 13. Vẽ ma trận nhầm lẫn ---
    plot_confusion_matrix(cf1)  # Vẽ ma trận nhầm lẫn